...
// Both helpers go through SpecForWithOverlay rather than SpecFor. SpecFor
// passes no trust anchors and no extension overlay, so every pooled connection was building a spec
...
func tcpClientHelloSpec(preset *fingerprint.Preset, id utls.ClientHelloID, seed int64) (*utls.ClientHelloSpec, error) {
	return fingerprint.SpecForWithOverlay(id, seed, preset.SignatureAlgorithms, preset.TrustAnchors, preset.ExtensionOverlay)
}

func quicClientHelloSpec(preset *fingerprint.Preset, id utls.ClientHelloID, seed int64) (*utls.ClientHelloSpec, error) {
	return fingerprint.SpecForWithOverlay(id, seed, preset.QUICSignatureAlgorithms, preset.TrustAnchors, preset.ExtensionOverlay)
}
